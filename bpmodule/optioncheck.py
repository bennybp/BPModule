...
def MergeAndCheckOptions(defaults, userset):
    ...
    if not IsValidDefaultTypes(defaults):
      raise exception.PyGeneralException("Developer error. Defaults don't all have a valid type! See above errors")


    # check for defaults for required options
    for k,v in defaults.items():
      if HasDefault(v[0]) and (v[1] == True):  # Required option, shouldn't have a default
        raise exception.PyGeneralException("Developer error. Required option {} given a default value".format(k))
    ...
    if ok == False:
        raise exception.PyGeneralException("Error parsing options")

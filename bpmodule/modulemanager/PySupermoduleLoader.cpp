...
PySupermoduleLoader::~PySupermoduleLoader()
{
    ...
    for(auto & it : objmap_)
    {
        output::GlobalOutput("Looking to close python supermodule %?\n", it.first);
...
const ModuleCreationFuncs & PySupermoduleLoader::LoadSupermodule(const std::string & spath)
{
    ...
    output::GlobalDebug("Loading python supermodule %?\n", spath);

 
    if(!objmap_.count(spath))
    {
        output::GlobalDebug("Supermodule not yet loaded. Looking to import python supermodule: %?\n", spath);
        ...
        output::GlobalDebug("Importing %?  from  %?\n", splitpath.first, splitpath.second);
        ...
        output::GlobalSuccess("Successfully imported supermodule %?\n", spath);
        ...
        if(python::HasCallableAttr(mod, "InitializeSupermodule"))
        {
            ...
            output::GlobalDebug("Supermodule %? doesn't have initialization function. Skipping\n", spath);

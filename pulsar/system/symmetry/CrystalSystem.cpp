...
CrystalSystem AssignCrystalSystem(const Vector_t& As,
    ...
    for(size_t i=0;i<3;++i){
        for(size_t j=i+1;j<3;++j){
            if(math::AreEqual(Ss[i],Ss[j],SideTol))--UniqueSides;
            if(math::AreEqual(As[i],As[j],AngleTol))--UniqueAngles;
        }
    }
    if(UniqueAngles==3&&UniqueSides==3)return Triclinic(As,Ss);
    int N90=0,N120=0;
    for(double Angle:As){
        if(math::AreEqual(90.0,Angle,90.0*Tol))++N90;
        else if(math::AreEqual(120.0,Angle,120.0*Tol))++N120;
    }
    if(UniqueAngles==0){
        if(N90==3){

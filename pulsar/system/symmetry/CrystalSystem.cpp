...
CrystalSystem AssignCrystalSystem(const Vector_t& As,
    ...
    double SideTol=max(Ss[0],Ss[1],Ss[2])*Tol;
    double AngleTol=max(As[0],As[1],As[2])*Tol;
    int UniqueSides=3,UniqueAngles=3;
    for(size_t i=0;i<3;++i){
        for(size_t j=i+1;j<3;++j){
            if(are_equal(Ss[i],Ss[j],SideTol))--UniqueSides;
            if(are_equal(As[i],As[j],AngleTol))--UniqueAngles;
        }
    }
    if(UniqueAngles==3&&UniqueSides==3)return Triclinic(As,Ss);
    int N90=0,N120=0;
    for(double Angle:As){
        if(are_equal(90.0,Angle,90.0*Tol))++N90;
        else if(are_equal(120.0,Angle,120.0*Tol))++N120;
    }
    if(UniqueAngles==0){
        if(N90==3){
